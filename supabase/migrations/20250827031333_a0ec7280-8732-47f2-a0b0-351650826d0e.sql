-- First, add the new columns to matches table without enum constraints
...
-- Update existing event types to match enum values before creating enum
UPDATE public.match_events 
SET event_type = CASE 
  WHEN event_type = 'Kartu Kuning' THEN 'yellow_card'
  WHEN event_type = 'goal' THEN 'goal'
  ELSE 'commentary'
END;
...
-- Now safely alter the column to use enum
